...
class WriteBatch {
    ...
    commit_(commitOptions) {
        ...
        if (!explicitTransaction && this._shouldCreateTransaction()) {
            logger_1.logger('WriteBatch.commit', requestTag, 'Using transaction for commit');
        ...
        logger_1.logger('WriteBatch.commit', requestTag, 'Sending %d writes', request.writes.length);
...
function writeBatchPkg(FirestoreType, DocumentReferenceType, validateDocumentReference) {
    let document = document_1.documentPkg(DocumentReferenceType);
...
exports.writeBatchPkg = writeBatchPkg;
