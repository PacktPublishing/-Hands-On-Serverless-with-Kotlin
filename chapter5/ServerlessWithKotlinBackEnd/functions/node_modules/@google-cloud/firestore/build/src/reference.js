...
class DocumentChange {
    ...
    isEqual(other) {
        ...
        return (is_1.default.instanceof(other, DocumentChange) && this._type === other._type &&
...
class Query {
    ...
    static _isDocumentSnapshot(fieldValuesOrDocumentSnapshot) {
        return (fieldValuesOrDocumentSnapshot.length === 1 &&
            is_1.default.instance(fieldValuesOrDocumentSnapshot[0], DocumentSnapshot));
    }
    /**
    ...
    _stream(queryOptions) {
        ...
        this._firestore.readStream('runQuery', request, requestTag, true)
            ...
            backendStream.on('error', err => {
                logger_1.logger('Query._stream', requestTag, 'Query failed with stream error:', err);
                stream.destroy(err);
            });
        ...
        return (doc1, doc2) => {
            ...
            for (let orderBy of orderBys) {
                ...
                if (path_1.FieldPath._DOCUMENT_ID.isEqual(orderBy.field)) {
                    ...
                    comp = order_1.default.compare(v1, v2);
                }
                if (comp !== 0) {
...
function referencePkg(FirestoreType) {
    Firestore = FirestoreType;
    let document = document_1.documentPkg(DocumentReference);
    DocumentSnapshot = document.DocumentSnapshot;
    Watch = watch_1.watchPkg(FirestoreType, DocumentChange, DocumentReference, DocumentSnapshot);
    WriteBatch =
        write_batch_1.writeBatchPkg(FirestoreType, DocumentReference, validateDocumentReference)
            .WriteBatch;
...
exports.referencePkg = referencePkg;
